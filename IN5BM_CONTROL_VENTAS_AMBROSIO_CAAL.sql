...
-- -----------------------------------------------------
DROP DATABASE IF EXISTS db_control_ventas_ambrosio;
CREATE DATABASE IF NOT EXISTS db_control_ventas_ambrosio;
USE db_control_ventas_ambrosio;
...
CALL sp_ventas_create("2021-02-02");
-- -----------------------------------------------------
...
BEGIN
    ...
    VALUES(_compra_id,_producto_id,_cantidad,total);
...
CREATE PROCEDURE sp_productos_create(
   ...
   /* IN _existencia INT,
    IN _precio_unitaro DECIMAL (10,2),
    IN _precop_docena DECIMAL(10,2),
    IN _precio_mayor DECIMAL(10,2)*/
)
BEGIN
...
BEGIN
    ...
    VALUES(_venta_id,_producto_id,cantidad);
...
-- -----------------------------------------------------
/*DELIMITER $$
DROP TRIGGER IF EXISTS tr_detalle_compra_after_update $$
CREATE TRIGGER tr_detalle_compra_after_update
AFTER UPDATE
ON detalle_compras
FOR EACH ROW
BEGIN
...
FOR EACH ROW
BEGIN
	UPDATE productos SET precio_unitario=(new.total/new.cantidad)*0.35
	WHERE id=NEW.producto_id;
    UPDATE productos SET precio_docena=(new.total/new.cantidad)*0.25
	WHERE id=NEW.producto_id;
    UPDATE productos SET precio_mayor=(new.total/new.cantidad)*0.15
	WHERE id=NEW.producto_id;
END$$
DELIMITER ;
...
FOR EACH ROW
BEGIN
	UPDATE productos SET existencia=existencia+NEW.cantidad
	WHERE id=NEW.producto_id;
END$$
DELIMITER ;
-- -------------------------------------------------------------------------
...
DELIMITER $$
DROP TRIGGER IF EXISTS tr_detalle_ventass_after_insert$$
CREATE TRIGGER tr_detalle_ventasas_after_insert
AFTER INSERT
ON detalle_ventas
FOR EACH ROW
BEGIN
